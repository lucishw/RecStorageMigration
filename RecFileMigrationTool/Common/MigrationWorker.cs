    ...
    public class MigrationWorker
    {
        ...
        private string BuildSourceFile(DataRow row, string fileName, string serverName, string recDate)
        {
            if (!String.IsNullOrWhiteSpace(_settings.I360HttpFullPathField))
            {
                ...
                MigrationPathMapping mapping = FindPathMappingByVirtualPath(sourceFile);
                if (mapping != null)
                    return ReplacePathPrefix(sourceFile, mapping.VirtualPath, mapping.RealSourcePath).Replace("/", @"\");

                throw new Exception("No Impact360 path mapping matched - " + sourceFile);
            }
        ...
        private string BuildTargetFile(string sourceFile)
        {
            if (!String.IsNullOrWhiteSpace(_settings.I360HttpFullPathField))
            {
                ...
                if (mapping != null)
                    return ReplacePathPrefix(sourceFile, mapping.RealSourcePath, _settings.TargetPath);
        ...
        {
            if (_settings.PathMappings == null || String.IsNullOrWhiteSpace(sourcePath))
                return null;

            MigrationPathMapping bestMapping = null;
            int bestLength = -1;

            foreach (MigrationPathMapping mapping in _settings.PathMappings)
            {
        ...
        private void MarkRow(DBManager dbManager, DataRow row, string markValue, string dialedNumber, string callerNumber)
        {
            try
            {
                ...
                string markTableName = BuildMarkTableName(year);

                StringBuilder query = new StringBuilder();

                query.Append("Update " + _settings.SourceDBNM + ".dbo." + markTableName);
        ...
        private string BuildMarkTableName(string year)
        {
            string tableName = _settings.SourceMarkTableName;

            if (String.IsNullOrWhiteSpace(tableName))
                tableName = _settings.SourceTableName;

            if (_settings.UseYearTableMarking && !String.IsNullOrWhiteSpace(year))
